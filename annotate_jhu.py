...
def main():
    ...
    for sub_dir_data in sub_dirs:
        ...
        new_path = f"./{new_parent_dir}/{sub_dir}"

        # Clear image folder if it exists
        rmtree(join(new_path, "images"), ignore_errors=True)
        ...
        with open(join(jhu_path, label_filename), "r") as label_file:
            ...
            with open(join(new_path, label_filename), "w") as new_label_file:

                filewriter = csv.writer(new_label_file)
                num_copy = 0
                ...
                for label in jhu_labels:
                    ...
                    if label[0] not in remove_list:  # check that image isn't removed
                        ...
                        filewriter.writerow([fn, 0])  # add to new labels

                        copy(
                            join(jhu_path, f"images/{label[0]}.jpg"),
                            join(new_path, f"images/{fn}.jpg"),
                        )
                        num_copy += 1
